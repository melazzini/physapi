...
namespace physapi
{
    ...
    phys_float PVerner::getSigma(const PVerner::Table1::const_iterator &it_Table1, phys_float e)
    {
        ...
        auto nz = it_Table1->first[(uint)eTable1Key::Z];
        auto ne = it_Table1->first[(uint)eTable1Key::Ne];
        auto is = it_Table1->first[(uint)eTable1Key::Is];

        //we need to update the array to
        //select the correct address of
        //the current element in m_Table2

        CurrentElementTable2[(uint)eTable2Key::Z] = nz;
        CurrentElementTable2[(uint)eTable2Key::Ne] = ne;
