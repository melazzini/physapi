...
namespace physapi
{
    ...
    ****************************************************************/
     ...
     */
    template <typename T_key, typename T_value, phys_size size, typename T_LoadArray>
    class PProcessTable
    {
        class t_Table_ : public std::vector<std::pair<T_key, T_value>>
        {
        public:
            void loadElement(const std::array<phys_float, size> &a)
            {
    ...
    public:
         ...
         * @param file file name
         */
        PProcessTable(const std::string &file) { loadFromFile<size>(file, m_Table); }
        ...
        {
            loadFromFile<size>(file, m_Table);
     ...
     */
    constexpr phys_float solidAngle(phys_float theta)
    {
